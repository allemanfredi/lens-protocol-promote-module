...
contract TimedPromoteModule is ModuleBase, IReferenceModule {
    ...
    function initializeReferenceModule(
        ...
        for (uint256 i = 0; i < tokens.length; ) {
            ...
            rewards[collectorProfileIds[i]][_pubId].push(Reward(tokens[i], _profileId, amounts[i], endTimestamps[i], Constants.NOT_COLLECTED));
    ...
    function processMirror(
        ...
        for (uint256 i = 0; i < rewardsByCollectorProfileIdLength; ) {
            ...
            uint64 endTimestamp = reward.endTimestamp;
            if (block.timestamp <= endTimestamp && reward.status == Constants.NOT_COLLECTED) {
                ...
                emit RewardCollected(_pubIdPointed, _collectorProfileId, _profileIdPointed, token, amount, endTimestamp);
            }
    ...
    function deleteRewards(uint256 _collectorProfileId, uint256 _pubId) external {
        ...
        for (uint256 index = 0; index < rewardsByCollectorProfileIdLength; ) {
            ...
            emit RewardDeleted(_pubId, _collectorProfileId, profileId, token, amount, reward.endTimestamp);
